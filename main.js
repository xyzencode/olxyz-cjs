...
const { smsg, Module } = require("./lib/serialize")
...
if (process.env.WRITE_STORE) store.readFromFile("./store.json");
...
const Connecting = async () => {
    ...
    const client = makeWASocket({
        version,
        logger,
        printQRInTerminal: !process.env.usePairingCode,
        auth: {
            creds: state.creds,
    ...
    if (process.env.usePairingCode && !client.authState.creds.registered) {
        console.clear();
        let number = process.env.NUMBER_BOT;
    ...
    client.ev.on("messages.upsert", async ({
        ...
        if (process.env.SELF && m.isOwner) return;

        (await require(`./message.js`))(client, store, m, messages[0]);
    });

    setInterval(async () => {
        if (process.env.WRITE_STORE) {
